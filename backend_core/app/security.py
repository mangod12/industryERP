...
def get_secret_key() -> str:
    ...
    if not secret:
        ...
        warnings.warn("⚠️  Using auto-generated secret key. Set KUMAR_SECRET_KEY for production!", RuntimeWarning)
...
ROLE_PERMISSIONS: dict[str, Set[str]] = {
    "Boss": {
        ...
        Permission.DISPATCH_APPROVE,
        Permission.QA_VIEW,
        Permission.QA_INSPECT,
        Permission.QA_APPROVE,
        Permission.QA_REJECT,
        Permission.QA_HOLD,
...
def require_permission(*required_permissions: str):
    ...
    async def permission_checker(current_user=Depends(get_current_user)):
        ...
        if missing:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f"Missing permissions: {', '.join(missing)}"
            )
...
class RateLimiter:
    ...
    @classmethod
    def check_rate_limit(cls, key: str, max_attempts: int = 5, window_seconds: int = 300) -> tuple[bool, int]:
        """
        ...
        if key in cls._attempts:
            cls._attempts[key] = [t for t in cls._attempts[key] if t > window_start]
        else:
            cls._attempts[key] = []
