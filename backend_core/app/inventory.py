...
def create_item(
    ...
    if item_in.used > item_in.total:
        raise HTTPException(status_code=400, detail="Used quantity cannot exceed total quantity")
    ...
    if existing:
        raise HTTPException(status_code=400, detail=f"Material '{item_in.name.strip()}' already exists in inventory")
...
def update_item(
    ...
    try:
        i = db.query(models.Inventory).filter(models.Inventory.id == item_id).with_for_update(nowait=True).first()
    except Exception:
        # Fallback for SQLite
    ...
    try:
        ...
        if old_total != i.total or old_used != i.used:
            print(
                f"[AUDIT] Item {item_id} updated by user {current_user.id}: "
...
def get_inventory_stats(
    ...
    recent_usage = db.query(models.MaterialUsage).order_by(models.MaterialUsage.ts.desc()).limit(10).all()
...
def reset_consumed(
    db: Session = Depends(get_db), current_user: models.User = Depends(require_role("Boss", "Software Supervisor"))
):
    """
    ...
    log = models.ActivityLog(
        action="RESET_CONSUMED", description="Reset Total Consumed (kg) for all items", user_id=current_user.id
    )
    db.add(log)
    db.commit()
...
def reset_stock(
    ...
    # Log activity
    log = models.ActivityLog(
        action="RESET_STOCK", description="Reset Total Stock (kg) for all items", user_id=current_user.id
    )
    db.add(log)
    db.commit()
...
def get_dashboard_data(
    ...
    for stage in ["fabrication", "painting", "dispatch"]:
        ...
        pending_sum = (
            db.query(func.coalesce(func.sum(models.ProductionItem.quantity), 0))
            .join(models.StageTracking)
    ...
    total_items = (
        db.query(func.coalesce(func.sum(models.ProductionItem.quantity), 0))
        .join(models.Customer)
        .filter(models.ProductionItem.is_archived == False, models.Customer.is_deleted == False)
        .scalar()
    )
    ...
    recent_stages = (
        ...
        .join(models.ProductionItem)
        .join(models.Customer)
        .filter(models.ProductionItem.is_archived == False, models.Customer.is_deleted == False)
        .order_by(models.StageTracking.id.desc())
        .limit(10)
        .all()
    )

    # Recent system logs (resets, etc.)
    recent_logs = db.query(models.ActivityLog).order_by(models.ActivityLog.timestamp.desc()).limit(5).all()

    recent_activity = []

    # Add stage activities
    for s in recent_stages:
        item = db.query(models.ProductionItem).filter(models.ProductionItem.id == s.production_item_id).first()
        if item:
            recent_activity.append(
                {
                    ...
                    "stage": s.stage,
                    "status": s.status,
                    "timestamp": (s.completed_at or s.started_at).isoformat()
                    if (s.completed_at or s.started_at)
                    else None,
                    "type": "tracking",
                }
            )
