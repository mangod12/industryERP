...
class MaterialMaster(Base):
    ...
    __table_args__ = (Index("ix_material_type_grade", "material_type", "grade"),)
...
class StockMovement(Base):
    ...
    # Valuation fields
    valuation_rate = Column(Numeric(15, 4), nullable=True)  # cost per kg at time of movement
    stock_value_change = Column(Numeric(15, 2), nullable=True)  # monetary value change
    balance_stock_value = Column(Numeric(15, 2), nullable=True)  # running total value after
    balance_qty_kg = Column(Numeric(15, 3), nullable=True)  # running quantity after
    posting_date = Column(Date, nullable=True)  # allows backdated entries
    fiscal_year = Column(String(10), nullable=True)  # e.g., "FY2526"
...
class NumberSequence(Base):
    ...
    format_str = Column(String(100), nullable=True)  # e.g., "{prefix}/{fy}/{####}"
