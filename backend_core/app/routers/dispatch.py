...
from ..models_v2 import DispatchLineItem, DispatchNote, DocumentStatus, MaterialMaster, QAStatus, StockLot
...
async def list_dispatches(
    ...
    """
    query = db.query(DispatchNote, Customer.name.label("customer_name")).join(
    ...
    results = query.order_by(DispatchNote.created_at.desc()).offset(offset).limit(limit).all()
...
async def create_dispatch(
    ...
    return {"success": True, "dispatch_id": dispatch.id, "dispatch_number": dispatch.dispatch_number}
...
async def add_dispatch_line_item(
    ...
    if dispatch.status != DocumentStatus.DRAFT:
        raise HTTPException(status_code=400, detail="Can only add items to draft dispatches")
    ...
    if lot.is_blocked:
        raise HTTPException(status_code=400, detail=f"Stock lot is blocked: {lot.block_reason}")

    if lot.qa_status not in (QAStatus.APPROVED, QAStatus.CONDITIONAL):
        raise HTTPException(status_code=400, detail="Stock lot is not QA approved")

    weight_kg = Decimal(str(data.weight_kg))

    # Check if lot already in this dispatch
    existing = (
        db.query(DispatchLineItem)
        .filter(DispatchLineItem.dispatch_id == dispatch_id, DispatchLineItem.stock_lot_id == data.stock_lot_id)
        .first()
    )
    ...
    if existing:
        ...
        db.commit()
        return {"success": True, "message": "Updated existing line item", "line_item_id": existing.id}

    # Check available stock
    if weight_kg > lot.current_weight_kg:
        raise HTTPException(
            status_code=400, detail=f"Requested weight ({weight_kg}) exceeds available ({lot.current_weight_kg})"
        )
    ...
    return {"success": True, "line_item_id": line.id, "lot_number": lot.lot_number}
...
async def auto_pick_material(
    ...
    for lot, weight_to_pick in picks:
        line = DispatchLineItem(dispatch_id=dispatch_id, stock_lot_id=lot.id, dispatched_weight_kg=weight_to_pick)
        db.add(line)
        db.flush()
...
async def record_dispatch_weighment(
    ...
    expected_weight = sum(line.dispatched_weight_kg for line in dispatch.line_items)
...
async def submit_dispatch(
    ...
    if not dispatch.line_items:
        raise HTTPException(status_code=400, detail="Dispatch must have at least one item")

    try:
        transition_document(db, dispatch, "submitted", current_user.role)
        dispatch.updated_at = datetime.utcnow()
        db.commit()
    except WorkflowError as e:
        raise HTTPException(status_code=400, detail=str(e))

    return {"success": True, "message": "Dispatch submitted for approval", "dispatch_number": dispatch.dispatch_number}
...
async def approve_dispatch(
    ...
    try:
        ...
        transition_document(db, dispatch, "approved", current_user.role, context)
...
async def cancel_dispatch(
    ...
    Only draft or submitted dispatches can be cancelled.
    Approved dispatches with deducted stock cannot be cancelled.
    """
    dispatch = db.query(DispatchNote).filter(DispatchNote.id == dispatch_id).first()
    ...
    try:
        context = {"dispatch_id": dispatch.id, "reason": reason}
        transition_document(db, dispatch, "cancelled", current_user.role, context)
        db.commit()
    except WorkflowError as e:
        raise HTTPException(status_code=400, detail=str(e))
    ...
    SecurityAuditLog.log_sensitive_action(
        ...
        "dispatch",
        dispatch.id,
        {"dispatch_number": dispatch.dispatch_number, "reason": reason},
    )

    return {"success": True, "message": "Dispatch cancelled", "dispatch_number": dispatch.dispatch_number}
...
async def remove_dispatch_line_item(
    ...
    if dispatch.status != DocumentStatus.DRAFT:
        raise HTTPException(status_code=400, detail="Can only modify draft dispatches")

    line = (
        db.query(DispatchLineItem)
        .filter(DispatchLineItem.id == line_item_id, DispatchLineItem.dispatch_id == dispatch_id)
        .first()
    )
