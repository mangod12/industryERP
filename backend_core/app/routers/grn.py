...
from ..services.inventory_service import GRNService
from ..services.workflow_engine import WorkflowError, transition_document
...
async def list_vendors(db: Session = Depends(get_db), current_user=Depends(require_permission(Permission.GRN_VIEW))):
...
async def create_vendor(
    ...
    vendor = Vendor(code=code, name=name, gstin=gstin, city=city, contact_person=contact_person, phone=phone)
    db.add(vendor)
    db.commit()
    db.refresh(vendor)
...
async def list_grns(
    ...
    """
    query = db.query(GoodsReceiptNote, Vendor.name.label("vendor_name")).join(
    ...
    results = query.order_by(GoodsReceiptNote.created_at.desc()).offset(offset).limit(limit).all()
...
async def create_grn(
    ...
    SecurityAuditLog.log_sensitive_action(
        db, current_user.id, "create", "grn", grn.id, {"grn_number": grn.grn_number, "vendor_id": data.vendor_id}
    )

    return {"success": True, "grn_id": grn.id, "grn_number": grn.grn_number, "status": grn.status.value}
...
async def add_grn_line_item(
    ...
    return {"success": True, "line_item_id": line.id, "message": "Line item added to GRN"}
...
async def record_weighment(
    ...
    if grn.status != DocumentStatus.DRAFT:
        raise HTTPException(status_code=400, detail="Weighment can only be recorded for draft GRNs")
...
async def submit_grn(
    grn_id: int, db: Session = Depends(get_db), current_user=Depends(require_permission(Permission.GRN_CREATE))
):
    """
    ...
    if not grn.line_items:
        raise HTTPException(status_code=400, detail="GRN must have at least one line item")

    if not grn.net_weight_kg:
        raise HTTPException(status_code=400, detail="Weighment data must be recorded before submission")

    try:
        transition_document(db, grn, "submitted", current_user.role)
        grn.updated_at = datetime.utcnow()
        db.commit()
    except WorkflowError as e:
        raise HTTPException(status_code=400, detail=str(e))

    return {"success": True, "message": "GRN submitted for approval", "status": grn.status.value}
...
async def record_qa_inspection(
    ...
    if grn.status != DocumentStatus.SUBMITTED:
        raise HTTPException(status_code=400, detail="GRN must be submitted before QA inspection")

    results = []

    for inspection in inspections:
        line = (
            db.query(GRNLineItem)
            .filter(GRNLineItem.id == inspection.line_item_id, GRNLineItem.grn_id == grn_id)
            .first()
        )

        if not line:
            results.append({"line_item_id": inspection.line_item_id, "success": False, "error": "Line item not found"})
            continue
        ...
        results.append({"line_item_id": inspection.line_item_id, "success": True, "status": inspection.status})
...
async def approve_grn(
    ...
    try:
        ...
        transition_document(db, grn, "approved", current_user.role, context)
        ...
        SecurityAuditLog.log_sensitive_action(
            ...
            "grn",
            grn.id,
            {"grn_number": grn.grn_number, "lots_created": len(lots), "lot_numbers": [lot.lot_number for lot in lots]},
        )
        ...
        return {
            ...
            "grn_number": grn.grn_number,
            "lots_created": [
                {"lot_number": lot.lot_number, "material_id": lot.material_id, "weight_kg": float(lot.net_weight_kg)}
                for lot in lots
            ],
        }
...
async def cancel_grn(
    ...
    try:
        context = {"grn_id": grn_id, "reason": reason}
        transition_document(db, grn, "cancelled", current_user.role, context)
        db.commit()
    except WorkflowError as e:
        raise HTTPException(status_code=400, detail=str(e))

    SecurityAuditLog.log_sensitive_action(
        db, current_user.id, "cancel", "grn", grn.id, {"grn_number": grn.grn_number, "reason": reason}
    )

    return {"success": True, "message": "GRN cancelled", "grn_number": grn.grn_number}
