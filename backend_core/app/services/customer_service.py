...
def hard_delete_customer(db: Session, customer: models.Customer, user: Optional[models.User] = None):
    ...
    try:
        # Stage tracking
        if item_ids:
            db.query(models.StageTracking).filter(models.StageTracking.production_item_id.in_(item_ids)).delete(
                synchronize_session=False
            )

            # Material usage
            if hasattr(models, "MaterialUsage"):
                db.query(models.MaterialUsage).filter(models.MaterialUsage.production_item_id.in_(item_ids)).delete(
                    synchronize_session=False
                )
            ...
            if hasattr(models, "MaterialConsumption"):
                ...
                try:
                    db.query(models.MaterialConsumption).filter(
                        models.MaterialConsumption.material_usage_id.in_(item_ids)
            ...
            if hasattr(models, "ScrapRecord"):
                db.query(models.ScrapRecord).filter(models.ScrapRecord.source_item_id.in_(item_ids)).delete(
                    synchronize_session=False
                )
            if hasattr(models, "ReusableStock"):
                db.query(models.ReusableStock).filter(models.ReusableStock.source_item_id.in_(item_ids)).delete(
                    synchronize_session=False
                )

            # Tracking history
            if hasattr(models, "TrackingStageHistory"):
                db.query(models.TrackingStageHistory).filter(
                    models.TrackingStageHistory.material_id.in_(item_ids)
                ).delete(synchronize_session=False)

            # Capture ExcelUpload ids referenced
            excel_rows = (
                db.query(models.ProductionItem.excel_upload_id)
                .filter(models.ProductionItem.id.in_(item_ids))
                .distinct()
                .all()
            )
            ...
            db.query(models.ProductionItem).filter(models.ProductionItem.id.in_(item_ids)).delete(
                synchronize_session=False
            )

            # Delete ExcelUpload records referenced by these items
            if excel_ids:
                db.query(models.ExcelUpload).filter(models.ExcelUpload.id.in_(excel_ids)).delete(
                    synchronize_session=False
                )
