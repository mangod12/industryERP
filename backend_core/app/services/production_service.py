...
from .. import models
from ..models import Customer, ExcelUpload, Inventory, MaterialUsage, Notification, ProductionItem, StageTracking
...
class ProductionService:
    ...
    def _find_inventory_match(profile: str, db: Session, item_code: str = None) -> Optional[Inventory]:
        ...
        if profile:
            mapping = (
                ...
                .filter(models.MaterialMapping.excel_name.ilike(profile.strip()))
                .first()
            )
            if mapping:
        ...
        def try_match(search_str: str):
            ...
            match = (
                db.query(Inventory)
                .filter(or_(Inventory.name.ilike(clean), Inventory.section.ilike(clean), Inventory.code.ilike(clean)))
                .first()
            )
            if match:
                return match
            ...
            for inv in all_inv:
                inv_norm = (
                    (inv.name or "").upper().replace("X", "*").replace("x", "*").replace(" ", "").replace("-", "")
                )
    ...
    def preview_production_excel(cls, db: Session, file_content: bytes, filename: str) -> Dict[str, Any]:
        ...
        for idx, row in df.iterrows():
            ...
            if section or code_val:
                inv = cls._find_inventory_match(
                    str(section) if section else None, db, item_code=str(code_val) if code_val else None
                )
                if inv:
    ...
    def process_production_excel(
        ...
        deduct_from_inventory_id: Optional[int] = None,
    ):
        """
        ...
        # 3. Process Rows
        stats = {"created": 0, "updated": 0, "skipped": 0, "errors": [], "grand_total_weight_kg": 0.0}
        ...
        existing_map = {(i.item_code or "").lower(): i for i in existing}
        # Also map by name if code missing
        existing_name_map = {(i.item_name or "").lower(): i for i in existing}
        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                try:
                    length = int(float(length)) if length else None
                except (ValueError, TypeError):
                    length = None
                ...
                if item:
                    ...
                    for stage_name in ["fabrication", "painting", "dispatch"]:
                        stage = StageTracking(
                            production_item_id=item.id, stage=stage_name, status="pending", updated_by=user_id
                        )
                        db.add(stage)
        ...
        if deduct_from_inventory_id and stats["grand_total_weight_kg"] > 0:
            ...
            if inventory_item:
                ...
                try:
                    print(
                        f"[BULK DEDUCTION] Deducted {stats['grand_total_weight_kg']:.2f} kg from {inventory_item.name}"
                    )
                except Exception:
                    pass
            else:
