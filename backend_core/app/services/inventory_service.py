...
from ..models_v2 import MaterialConsumptionV2 as MaterialConsumption
...
def get_indian_fiscal_year(date=None) -> str:
    ...
    Jan 2026 - Mar 2026 = 'FY2526' (still previous FY)
    Apr 2026 - Mar 2027 = 'FY2627'
    """
    d = date or datetime.utcnow().date()
    if hasattr(d, "date"):
        d = d.date()  # Handle datetime objects
    if d.month >= 4:
...
def _apply_format(format_str: str, prefix: str, padding: int, number: int, ref_date=None) -> str:
    """
    Apply a format string to produce the final sequence value.

    Tokens:
      {prefix}  - sequence prefix
      {year}    - calendar year (2026)
      {fy}      - Indian fiscal year (FY2526 for Apr 2025 - Mar 2026)
      {month}   - zero-padded month (01-12)
      {####}    - padded number (# count = padding width)
    """
    ...
    result = result.replace("{year}", str(d.year))
    result = result.replace("{fy}", get_indian_fiscal_year(d))
    result = result.replace("{month}", f"{d.month:02d}")

    # Handle {####} pattern — count the #s for padding width
    import re

    def _replace_hashes(match):
        hash_count = len(match.group(0)) - 2  # subtract the { and }
        return str(number).zfill(hash_count)

    result = re.sub(r"\{#+\}", _replace_hashes, result)
...
def get_next_sequence(
    ...
    """
    Get next number in sequence with configurable format.
    ...
    # Determine effective format string
    effective_format = format_str or seq.format_str
    effective_prefix = seq.prefix or prefix

    if effective_format:
        return _apply_format(effective_format, effective_prefix, seq.padding, seq.current_number)

    # Default format: PREFIX/YEAR/NUMBER or PREFIX/NUMBER
...
class StockLotService:
    ...
    @staticmethod
    def create_lot_from_grn(db: Session, grn_line: GRNLineItem, location_id: Optional[int], user_id: int) -> StockLot:
        """
    ...
    def consume_from_lot(
        ...
        lot = db.query(StockLot).filter(StockLot.id == lot_id).with_for_update().first()
    ...
    @staticmethod
    def adjust_stock(
        db: Session, lot_id: int, new_weight_kg: Decimal, user_id: int, reason: str, approved_by: Optional[int] = None
    ) -> Tuple[StockMovement, StockLot]:
        """
    ...
    def transfer_location(
        db: Session, lot_id: int, to_location_id: int, user_id: int, reason: Optional[str] = None
    ) -> Tuple[StockMovement, StockLot]:
    ...
    @staticmethod
    def split_lot(
        db: Session, lot_id: int, split_weights_kg: List[Decimal], user_id: int, reason: str
    ) -> List[StockLot]:
        """
        ...
        for weight_kg in split_weights_kg:
            ...
            db.add(movement_in)
            db.flush()
            StockValuationService.record_valuation_on_movement(db, movement_in, new_lot)
            if ACCOUNTING_ENABLED:
                AccountingService.create_entry_for_stock_movement(db, movement_in)
        ...
        db.add(movement_out)
        db.flush()
        StockValuationService.record_valuation_on_movement(db, movement_out, lot)
        if ACCOUNTING_ENABLED:
            AccountingService.create_entry_for_stock_movement(db, movement_out)
...
class InventoryQueryService:
    ...
    @staticmethod
    def get_stock_aging_report(db: Session, days_threshold: int = 90) -> List[dict]:
        """
        ...
        query = (
            db.query(StockLot, MaterialMaster.code, MaterialMaster.name)
            .join(MaterialMaster, StockLot.material_id == MaterialMaster.id)
    ...
    def get_lots_for_fifo_pick(
        db: Session, material_id: int, required_weight_kg: Decimal, location_id: Optional[int] = None
        ...
        if remaining > 0:
            raise InsufficientStockError(f"Cannot fulfill requirement. Short by {remaining} kg")

        return picks

    @staticmethod
    def reconcile_physical_vs_system(db: Session, lot_id: int, physical_weight_kg: Decimal) -> dict:
        """
...
class GRNService:
    ...
    @staticmethod
    def approve_grn(
        db: Session, grn_id: int, user_id: int, location_id: int
