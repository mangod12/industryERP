...
r = SESSION.post(
    f"{BASE}/customers",
    headers=HEADERS,
    json={"name": "Acme Steel Works", "project_details": "Factory extension project"},
)
...
r = SESSION.put(
    ...
    headers=HEADERS,
    json={"name": "Acme Steel Works Pvt Ltd", "project_details": "Updated project details"},
)
assert_ok(r, "Update customer")

# Create another customer for testing
r = SESSION.post(
    f"{BASE}/customers", headers=HEADERS, json={"name": "Beta Fabricators", "project_details": "Bridge project"}
)
...
# Add assembly
r = SESSION.post(
    f"{BASE}/api/v3/drawings/{dwg_id}/assemblies",
    headers=HEADERS,
    json={"mark_number": "HR-A1", "description": "Handrail main frame — left side", "quantity_required": 2},
)
...
r = SESSION.post(
    f"{BASE}/api/v3/drawings/{dwg_id}/assemblies",
    headers=HEADERS,
    json={"mark_number": "HR-A2", "description": "End post assembly", "quantity_required": 4},
)
...
r = SESSION.post(
    f"{BASE}/api/v3/drawings/assemblies/{asm_id}/components",
    headers=HEADERS,
    json={
        "piece_mark": "P1",
...
r = SESSION.post(
    ...
    json={
        "piece_mark": "P3",
        "profile_section": "6mm Base Plate",
        "length_mm": 300,
        "width_mm": 300,
        "thickness_mm": 6,
...
r = SESSION.post(
    f"{BASE}/api/v3/drawings/assemblies/{asm2_id}/components",
    headers=HEADERS,
    json={
        "piece_mark": "EP1",
        "profile_section": "75x75x6 Angle",
        "grade": "S275",
        "length_mm": 1000,
        "quantity_per_assembly": 1,
        "weight_each_kg": 6.8,
...
if assert_ok(r, "Get kanban board"):
    ...
    log("Cutting column has 16 instances", cutting_col and cutting_col["count"] == 16)
...
if len(instance_ids) >= 4:
    ...
    # Advance to drilling
    r = SESSION.post(
        f"{BASE}/api/v3/drawings/instances/{iid}/advance",
        headers=HEADERS,
        json={"component_instance_id": iid, "remarks": "Cutting complete"},
    )
    ...
    r = SESSION.post(
        f"{BASE}/api/v3/drawings/instances/{iid2}/advance",
        headers=HEADERS,
        json={"component_instance_id": iid2, "remarks": "Cut done"},
    )
    ...
    r = SESSION.post(
        f"{BASE}/api/v3/drawings/instances/{iid3}/hold",
        headers=HEADERS,
        json={"reason": "Material defect found — waiting for QA"},
    )
    assert_ok(r, f"Hold instance #{iid3}")

    # Hold without reason — should fail
    r = SESSION.post(f"{BASE}/api/v3/drawings/instances/{iid3}/hold", headers=HEADERS, json={"reason": ""})
    ...
    r = SESSION.post(
        f"{BASE}/api/v3/drawings/instances/{iid4}/scrap",
        headers=HEADERS,
        json={"reason": "Cutting error — piece too short"},
    )
    assert_ok(r, f"Scrap instance #{iid4}")

    # Scrap without reason — should fail
    r = SESSION.post(f"{BASE}/api/v3/drawings/instances/{iid4}/scrap", headers=HEADERS, json={"reason": ""})
    log("Scrap without reason fails", r.status_code in (400, 422))

    # Batch advance
    batch_ids = instance_ids[0:2]  # advance the two that are in drilling
    r = SESSION.post(
        f"{BASE}/api/v3/drawings/instances/batch-advance",
        headers=HEADERS,
        json={
    ...
    # Reserve materials
    r = SESSION.post(f"{BASE}/api/v3/drawings/{dwg_id}/reserve-materials", headers=HEADERS, json={"drawing_id": dwg_id})
    ...
    r = SESSION.post(
        f"{BASE}/api/v3/drawings/instances/{iid}/advance",
        headers=HEADERS,
        json={"component_instance_id": iid, "remarks": "Fitting done"},
    )
    ...
    r = SESSION.post(
        f"{BASE}/api/v3/drawings/instances/{iid}/advance",
        headers=HEADERS,
        json={"component_instance_id": iid, "remarks": "Welding done"},
    )
...
if instance_ids:
    r = SESSION.post(
        f"{BASE}/api/v3/drawings/instances/{instance_ids[0]}/advance",
        headers=HEADERS,
        json={"component_instance_id": instance_ids[0], "target_stage": "completed"},
    )
    log("Skip to completed blocked", r.status_code == 400)

# Advance nonexistent instance
r = SESSION.post(
    f"{BASE}/api/v3/drawings/instances/99999/advance", headers=HEADERS, json={"component_instance_id": 99999}
)
...
r = SESSION.post(
    f"{BASE}/queries/",
    headers=HEADERS,
    json={"title": "Material shortage", "message": "Running low on UB203 for staircase project"},
)
...
r = SESSION.post(
    f"{BASE}/queries/{query_id}/reply",
    headers=HEADERS,
    json={"reply": "Order placed, arriving Thursday", "status": "CLOSED"},
)
...
r = SESSION.post(
    f"{BASE}/instructions",
    headers=HEADERS,
    json={"message": "Priority: Complete all staircase handrail cutting by EOD"},
)
...
r = SESSION.put(
    f"{BASE}/instructions/{instr_id}",
    headers=HEADERS,
    json={"message": "UPDATED: Complete handrail cutting by tomorrow noon"},
)
...
r = SESSION.put(
    f"{BASE}/users/me",
    headers=HEADERS,
    json={"username": "boss", "email": "boss@kumarbros.com", "company": "Kumar Brothers Steel Pvt Ltd"},
)
...
r = SESSION.post(
    f"{BASE}/api/v3/drawings/",
    headers=HEADERS,
    json={"drawing_number": "<script>alert(1)</script>", "title": "XSS Test", "customer_id": cust_id},
)
if r.status_code in (200, 201):
    xss_data = r.json()
...
r = SESSION.post(
    f"{BASE}/api/v3/drawings/",
    headers=HEADERS,
    json={"drawing_number": "A" * 500, "title": "B" * 1000, "customer_id": cust_id},
)
...
r = (
    SESSION.post(
        f"{BASE}/api/v3/drawings/assemblies/{asm_id}/components",
        headers=HEADERS,
        json={"piece_mark": "NEG", "profile_section": "test", "quantity_per_assembly": 1, "weight_each_kg": -10},
    )
    if "asm_id" in dir()
    else None
)
...
r = (
    SESSION.post(
        f"{BASE}/api/v3/drawings/assemblies/{asm_id}/components",
        headers=HEADERS,
        json={"piece_mark": "ZERO", "profile_section": "test", "quantity_per_assembly": 0, "weight_each_kg": 10},
    )
    if "asm_id" in dir()
    else None
)
