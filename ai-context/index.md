...
- Customer list loading is now explicitly exposed as `window.applyCustomerFilters`, shows loading/error table states, and is covered by Playwright through the upload-template flow. Production smoke tests validate response contracts for inventory, GRN, and dispatch without assuming seed rows exist.
